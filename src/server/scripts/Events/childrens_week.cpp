        ...
        bool OnTrigger(Player* player, AreaTriggerEntry const* areaTrigger, bool /*entered*/) override
        {
            ...
            switch (areaTrigger->ID)
            {
                ...
                case AT_STONEWROUGHT_DAM:
                    ...
                    break;
                case AT_DARK_PORTAL:
                    questId = player->GetPlayerFaction() == ALLIANCE ? QUEST_DARK_PORTAL_A : QUEST_DARK_PORTAL_H;
                    orphanId = player->GetPlayerFaction() == ALLIANCE ? ORPHAN_DRAENEI : ORPHAN_BLOOD_ELF;
                    break;
            }
                        ...
                        if (player->HasAura(SPELL_ORPHAN_OUT))
                        {
                            ...
                            switch (me->GetEntry())
                            {
                                case NPC_CAVERNS_OF_TIME_CW_TRIGGER:
                                    questId = player->GetPlayerFaction() == ALLIANCE ? QUEST_TIME_TO_VISIT_THE_CAVERNS_A : QUEST_TIME_TO_VISIT_THE_CAVERNS_H;
                                    orphanId = player->GetPlayerFaction() == ALLIANCE ? ORPHAN_DRAENEI : ORPHAN_BLOOD_ELF;
                                    break;
