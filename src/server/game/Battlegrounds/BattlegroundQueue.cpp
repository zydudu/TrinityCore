...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
    ...
    ginfo->Team                      = leader->GetPlayerFaction();
...
void BattlegroundQueue::PlayerInvitedToBGUpdateAverageWaitTime(GroupQueueInfo* ginfo, BattlegroundBracketId bracket_id)
{
    ...
    uint8 team_index = BATTLEGROUND_TEAM_ALLIANCE_GOLD;                    //default set to TEAM_ALLIANCE - or non rated arenas!
    if (!ginfo->ArenaType)
    {
        if (ginfo->Team == HORDE)
            team_index = BATTLEGROUND_TEAM_HORDE_GREEN;
    }
    else
    {
        if (ginfo->IsRated)
            team_index = BATTLEGROUND_TEAM_HORDE_GREEN;                     //for rated arenas use TEAM_HORDE
    }
...
void BattlegroundQueue::FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id)
{
    ...
    uint32 aliIndex = 0;
    for (; aliIndex < aliCount && m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].AddGroup((*Ali_itr), aliFree); aliIndex++)
        ++Ali_itr;
    //the same thing for horde
    GroupsQueueType::const_iterator Horde_itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE].begin();
    uint32 hordeCount = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE].size();
    uint32 hordeIndex = 0;
    for (; hordeIndex < hordeCount && m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].AddGroup((*Horde_itr), hordeFree); hordeIndex++)
        ++Horde_itr;
    ...
    */
    ...
    int32 diffAli   = aliFree   - int32(m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount());
    int32 diffHorde = hordeFree - int32(m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount());
    while (abs(diffAli - diffHorde) > 1 && (m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() > 0 || m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount() > 0))
    {
        //each cycle execution we need to kick at least 1 group
        if (diffAli < diffHorde)
        {
            //kick alliance group, add to pool new group if needed
            if (m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].KickGroup(diffHorde - diffAli))
            {
                for (; aliIndex < aliCount && m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].AddGroup((*Ali_itr), (aliFree >= diffHorde) ? aliFree - diffHorde : 0); aliIndex++)
                    ++Ali_itr;
            }
            //if ali selection is already empty, then kick horde group, but if there are less horde than ali in bg - break;
            if (!m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount())
            {
                if (aliFree <= diffHorde + 1)
                    break;
                m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].KickGroup(diffHorde - diffAli);
            }
        }
        else
        {
            //kick horde group, add to pool new group if needed
            if (m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].KickGroup(diffAli - diffHorde))
            {
                for (; hordeIndex < hordeCount && m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].AddGroup((*Horde_itr), (hordeFree >= diffAli) ? hordeFree - diffAli : 0); hordeIndex++)
                    ++Horde_itr;
            }
            if (!m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount())
            {
                if (hordeFree <= diffAli + 1)
                    break;
                m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].KickGroup(diffAli - diffHorde);
            }
        }
        //count diffs after small update
        diffAli   = aliFree   - int32(m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount());
        diffHorde = hordeFree - int32(m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount());
    }
}
...
bool BattlegroundQueue::CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam)
{
    ...
    if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].empty() && !m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].empty())
    {
        ...
        {
            m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].AddGroup((*ali_group), MaxPlayersPerTeam);
            m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].AddGroup((*horde_group), MaxPlayersPerTeam);
            //add groups/players from normal queue to size of bigger group
            uint32 maxPlayers = std::min(m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount(), m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount());
            GroupsQueueType::const_iterator itr;
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
            {
...
bool BattlegroundQueue::CheckNormalMatch(Battleground* /*bg_template*/, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers)
{
    ...
    uint32 j = BATTLEGROUND_TEAM_ALLIANCE_GOLD;
    if (m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() < m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount())
        j = BATTLEGROUND_TEAM_HORDE_GREEN;
    if (sWorld->getIntConfig(CONFIG_BATTLEGROUND_INVITATION_TYPE) != 0
        && m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() >= minPlayers && m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount() >= minPlayers)
    {
        ...
        if (abs((int32)(m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() - m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount())) > 2)
            return false;
    }
    //allow 1v0 if debug bg
    if (sBattlegroundMgr->isTesting() && (m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount() || m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount()))
        return true;
    //return true if there are enough players in selection pools - enable to work .debug bg command correctly
    return m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount() >= minPlayers && m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() >= minPlayers;
}
...
{
    if (m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].GetPlayerCount() < minPlayersPerTeam && m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() < minPlayersPerTeam)
        return false;
    uint32 teamIndex = BATTLEGROUND_TEAM_ALLIANCE_GOLD;
    uint32 otherTeam = BATTLEGROUND_TEAM_HORDE_GREEN;
    uint32 otherTeamId = HORDE;
    if (m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].GetPlayerCount() == minPlayersPerTeam)
    {
        teamIndex = BATTLEGROUND_TEAM_HORDE_GREEN;
        otherTeam = BATTLEGROUND_TEAM_ALLIANCE_GOLD;
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    for (BGFreeSlotQueueContainer::iterator itr = bgQueues.begin(); itr != bgQueues.end();)
    {
            ...
            bg->GetStatus() > STATUS_WAIT_QUEUE && bg->GetStatus() < STATUS_WAIT_LEAVE)
        {
            ...
            m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].Init();
            m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].Init();

            // call a function that does the job for us
            FillPlayersToBG(bg, bracket_id);

            // now everything is set, invite players
            for (GroupsQueueType::const_iterator citr = m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].SelectedGroups.begin(); citr != m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->Team);

            for (GroupsQueueType::const_iterator citr = m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].SelectedGroups.begin(); citr != m_SelectionPools[BATTLEGROUND_TEAM_HORDE_GREEN].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->Team);
    ...
    if (bg_template->isBattleground())
    {
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
                for (GroupsQueueType::const_iterator citr = m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD + i].SelectedGroups.begin(); citr != m_SelectionPools[BATTLEGROUND_TEAM_ALLIANCE_GOLD + i].SelectedGroups.end(); ++citr)
    ...
    else if (bg_template->isArena())
    {
        ...
        if (found == 2)
        {
            GroupQueueInfo* aTeam = *itr_teams[BATTLEGROUND_TEAM_ALLIANCE_GOLD];
            GroupQueueInfo* hTeam = *itr_teams[BATTLEGROUND_TEAM_HORDE_GREEN];
            ...
            if (aTeam->Team != ALLIANCE)
            {
                ...
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].erase(itr_teams[BATTLEGROUND_TEAM_ALLIANCE_GOLD]);
            }
            if (hTeam->Team != HORDE)
            {
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].push_front(hTeam);
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].erase(itr_teams[BATTLEGROUND_TEAM_HORDE_GREEN]);
            }
